...
#TODO: file extensions for external links?
...
check_file_ext <- function(metadata_resource,
  ...
  if (is_blank(field_format) & !is_blank(file_ext)) {
    ...
    out <- c("FAIL", glue("The field_format ({field_format}) does not match the resource's file ext ({file_ext})"))
  }
