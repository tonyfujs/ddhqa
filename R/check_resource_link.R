...
check_resource_link <- function(metadata_resource) {
  ...
  if (code == "200") {
    out <- c("resource", resource_nid, "check_resource_links", "PASS", glue("{code}, you're good to go"))
  } else {
    out <- c("resource", resource_nid, "check_resource_links", "FAIL", glue("{code}, check the link"))
  }
